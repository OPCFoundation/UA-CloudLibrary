...
namespace CloudLibClient.Tests
{
    ...
    : IClassFixture<CustomWebApplicationFactory<Opc.Ua.Cloud.Library.Startup>>
    {
        ...
        public async Task Search(string[] keywords, int expectedCount)
        {
            ...
            ICollection<UANodesetResult> nodeSets = await PagedVsNonPagedAsync(apiClient, keywords: keywords, after: null, first: 100).ConfigureAwait(true);
            ...
            Assert.Equal(expectedCount, nodeSets.DistinctBy(n => n.NameSpaceUri).Count());
        }

        private async Task<ICollection<UANodesetResult>> PagedVsNonPagedAsync(UACloudLibClient apiClient, string[] keywords, string after, int first)
        {
            List<UANodesetResult> unpagedResult = await apiClient.GetBasicNodesetInformationAsync(int.Parse(after, CultureInfo.InvariantCulture), first, keywords.ToList()).ConfigureAwait(true);
            var unpaged = unpagedResult;

            List<UANodesetResult> paged = await GetAllPaged(apiClient, keywords: keywords, after: after, first: 5).ConfigureAwait(true);
            Assert.True(paged.Count == unpaged.Count);
            Assert.Equal(unpaged, paged, new NodesetComparer(output));

            var unpagedOrdered = unpaged.OrderBy(nsm => nsm.NameSpaceUri).ThenBy(nsm => nsm.PublicationDate).ToList();
            Assert.Equal(unpagedOrdered, paged, new NodesetComparer(output));

            return unpaged;
        }

        private static async Task<List<UANodesetResult>> GetAllPaged(UACloudLibClient apiClient, string[] keywords, string after, int first)
        {
            bool bComplete = false;
            var paged = new List<UANodesetResult>();
            int cursor = int.Parse(after, CultureInfo.InvariantCulture);

            do
            {
                List<UANodesetResult> page = await apiClient.GetBasicNodesetInformationAsync(int.Parse(after, CultureInfo.InvariantCulture), first, keywords.ToList()).ConfigureAwait(true);
    ...
    internal sealed class NodesetComparer : IEqualityComparer<UANodesetResult>
    {
        ...
        public bool Equals(UANodesetResult x, UANodesetResult y)
        {
        ...
        public int GetHashCode([DisallowNull] UANodesetResult p)
        {

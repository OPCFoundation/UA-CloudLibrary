...
namespace CloudLibClient.Tests
{
    ...
    : IClassFixture<CustomWebApplicationFactory<Opc.Ua.Cloud.Library.Startup>>
    {
        ...
        public async Task Search(string[] keywords, int expectedCount)
        {
            ...
            var nodeSets = await PagedVsNonPagedAsync(apiClient, keywords: keywords, after: null, first: 100).ConfigureAwait(true);
        ...
        {
            var unpagedResult = await apiClient.GetNodeSetsAsync(keywords: keywords, after: after, first: first).ConfigureAwait(true);
        ...
        private static async Task<List<Nodeset>> GetAllPaged(UACloudLibClient apiClient, string[] keywords, string after, int first)
        {
            ...
            do
            {
                var page = await apiClient.GetNodeSetsAsync(keywords: keywords, after: cursor, first: first).ConfigureAwait(true);

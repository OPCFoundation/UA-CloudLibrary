...
| **Large (>1M rows)** | **10× or more** in the common case: anonymous predicate collapses to a single index-only scan on `IX_NamespaceMeta_Published_Nodeset` instead of scanning `NamespaceMeta` and filtering. | **~5–10× faster** for filtered "my uploads" — partial `IX_NamespaceMeta_UserId` is small and cache-hot. | Unchanged (already O(1) via PK/unique index). |

Write path costs:
- Uploads/publishes: **negligible** overhead from the two partial indexes (they only include the subset each write touches).

A materialized view for the anonymous public catalog was evaluated and deferred to a later tuning pass.
...
- `Startup.cs` — added `EnsureIsPublishedColumnAsync`; wired into `CloudLibStartupTask`.

...
> **Audit durability limitation.** A DPP mutation touches the OPC UA address space, the version archive and the audit table &mdash; three stores with no shared transaction. Mutations therefore write an `Attempted` audit entry *before* the change and a `Success` entry after it. Both rows carry the same `OperationId` (migration `AddDppAuditOperationId`), which is part of the hashed content: without it the two rows could not be paired when operations on the same DPP interleave, and uploads could not be paired at all, because the attempt is recorded against the namespace URI while the outcome is recorded against the identifier assigned during the upload. A failed pre-write append refuses the request before anything is modified; an `Attempted` entry whose `OperationId` has no matching outcome means a change may have completed without being fully logged, and is the signal to investigate. This makes the gap **detectable**, not impossible: genuine atomicity requires a transactional outbox, which is not yet implemented.

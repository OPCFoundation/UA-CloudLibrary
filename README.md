...
| `granularity` | [1] | Enumeration: `model`, `batch`, `item` (EN 18223 Clause 4.1.2.2 â€” lowercase on the wire). |
...
> **A mapping that cannot be read unambiguously denies access rather than granting it.** The reserved property is matched case-insensitively, but JSON permits several spellings to coexist, so a document carrying both `controlledElements` and `ControlledElements` has no single authoritative policy. Taking whichever came first would let an empty decoy shadow a real mapping and publish the elements it protects, so that state is reported as invalid (deny-all) and a values rewrite refuses rather than dropping one of the copies. The same principle applies to an unparseable blob and to `"controlledElements": null`: an access mapping that cannot be understood is never treated as an absent one.

> **Historical reads apply the policy of their own version.** The values blob only ever describes the DPP as it is *now*, so filtering an archived version against it would disclose an element that was controlled at the requested date and un-mapped since &mdash; the access decision would silently follow the newest policy rather than the one that governed the data. Each archived snapshot therefore stores the mapping that was in force when it was captured, and `GET v1/dpps/{dppId}/versions/{date}` filters against that. Snapshots written before the archive recorded policy carry none; their policy cannot be reconstructed after the fact, so those versions return the DPP's public envelope with **no** data elements rather than guessing. Re-archiving is the only way to restore element-level history for them.

`VerifyChainAsync` re-walks the chain and additionally compares it against a persisted checkpoint (`DppAuditCheckpoints`, migration `AddDppAuditCheckpointAndEsdcSigningKey`) so that deleting the most recent entries &mdash; which would otherwise leave a still-valid prefix &mdash; is also detected. The checkpoint carries its own keyed MAC over the chain length and tail hash (`CheckpointMac`, migration `AddDppAuditCheckpointMac`), because an unauthenticated checkpoint could itself be restated to match a truncated log.

> **Audit durability limitation.** A DPP mutation touches the OPC UA address space, the version archive and the audit table &mdash; three stores with no shared transaction. Mutations therefore write an `Attempted` audit entry *before* the change and a closing entry after it. Both rows carry the same `OperationId` (migration `AddDppAuditOperationId`), which is part of the hashed content: without it the two rows could not be paired when operations on the same DPP interleave, and uploads could not be paired at all, because the attempt is recorded against the namespace URI while the outcome is recorded against the identifier assigned during the upload. **Every branch that knows how the operation ended writes a closing entry**, so an unmatched `Attempted` row means specifically that the outcome is *unknown* &mdash; the process died mid-operation, or the closing append itself failed &mdash; rather than merely that the request was refused. The closing outcomes are `Success`, `Failed` (the change did not happen), `Rejected` and `NotFound` (client errors), `Denied` (authorization refused), and `PartialFailure` (the change partly landed and could not be fully undone; needs operator attention). A failed pre-write append refuses the request before anything is modified. This makes the gap **detectable**, not impossible: genuine atomicity requires a transactional outbox, which is not yet implemented.
...
Entries written before any key was configured carry no fingerprint and keep verifying as unkeyed, so enabling a key for the first time needs no backfill. It does, however, need one deliberate step: the pre-existing checkpoint is unkeyed, and the server will not adopt it automatically (see below). Set `Dpp__Audit__AllowUnkeyedCheckpointMigration` to `true` for the first keyed run, then remove it once the checkpoint has been re-keyed by the first audited operation.

> **An unkeyed checkpoint is refused once keyed auditing is enabled, unless the migration is explicitly enabled.** The checkpoint lives in the database it protects, so "no key was used here" is itself an attacker-controllable claim: clearing its key id and MAC would otherwise skip MAC verification entirely and re-open truncation to a valid prefix even with a key configured. This cannot be resolved by inspecting the log, because `KeyId` is an ordinary column in the very table the key protects &mdash; an attacker who rewrites history can also clear every entry's fingerprint and reproduce the "genuinely pre-key" shape exactly, and the next honest append would then re-MAC the forged chain. Accepting an unkeyed checkpoint is therefore an explicit operator decision (`Dpp__Audit__AllowUnkeyedCheckpointMigration`) rather than an inference drawn from data that is under the attacker's control. Leave it off in steady state: with it off, an unkeyed checkpoint fails verification and blocks appends. The key id is also covered by the checkpoint MAC, so a checkpoint cannot be repointed at a different key while keeping its MAC intact.

> **A missing checkpoint blocks further appends when entries already exist.** Deleting the checkpoint together with a suffix of entries would otherwise be laundered by the next legitimate write: re-creating the checkpoint from whatever survives MACs the truncated prefix, permanently authenticating it before `GET /health/dpp-audit` ever runs. The audit log therefore refuses to initialize a checkpoint while entries are present, and the originating operation fails with `503` (the same refusal as any other unauditable request). **This is deliberately disruptive** &mdash; DPP mutations stop until an operator restores the checkpoint from backup or deliberately re-initializes the audit log, because continuing would mean silently certifying a log that may have been truncated. A checkpoint is created automatically only on a genuinely empty log.
...
An ESDC asserts *who* issued a passport, but the `issuer` claim is copied from the DPP's own `EconomicOperatorId` &mdash; content this server merely hosts, not a statement about who holds the signing key. Nothing in the document itself prevents a passport naming one operator from being signed by a server belonging to another.

Set `Dpp__Esdc__EconomicOperatorId` to the operator this deployment actually represents. Two things then hold:

* **Issuance is refused** for any DPP whose `EconomicOperatorId` differs, rather than producing a credential that asserts an identity this key does not hold.
* **The server's own key is bound** to that identifier as a trust anchor, so a credential naming a different operator cannot be verified against it either.

> **This setting is required wherever the signing key is.** An optional guard against impersonation is no guard at all for the deployment that forgets it, so a server resolving a stable signing key refuses to start without an operator id &mdash; the same treatment `Dpp__Esdc__PrivateKeyPem` already gets, and for the same reason: signing with an unbound issuer is a downgrade that has to be a deliberate decision rather than an oversight. **Existing deployments must set it as part of the upgrade**, otherwise the server will not start. Peer operators' keys should likewise use the issuer-bound `Dpp__Esdc__TrustedIssuers__*` form rather than the legacy flat `Dpp__Esdc__TrustedPublicKeysPem__*` list, in which any key can vouch for any issuer.
...
**EN 18239 â€” Access rights management, IT security, business confidentiality**

| Clause | Requirement | Status |
|---|---|---|
| Â§5.2(2), Â§6.1/6.2 | Public data readable without authentication; unauthenticated public vs. authenticated controlled access schemes | Implemented |
| Â§5.2(1)/(4)/(8) | Globally unique operator identifier available for identification/authorization/accounting | Implemented |
| Â§5.2(5)/(16) | Non-repudiation; all access and role/rights changes logged and tamper-evident over time | Implemented |
| Â§5.2(10)/(23) | Access rights enforceable at controlled data-element granularity per role | Implemented |
| Â§5.2(11)/(15) | Limit access to prevent attacks / unauthorized mass data scraping (rate limiting) | Implemented |
| Â§5.2(17)/(19), Â§6.3 | Granting/modifying/revoking access when roles change; emergency revocation; access-revocation policy | Implemented |
| Â§5.2(22) | Controlled data authenticated and authorized before access; least privilege | Implemented |
| Â§5.2(24) | Profiling of users avoided | Implemented |
| Â§5.2(18)/(20) | Delegation of roles (authorization also considering the delegating entity's role) | Deployment |
| Â§5.2(13), Annex A | Authentication per the relevant legal act; MFA, identity proofing, sole control, dynamic auth | Deployment |
| Â§6.4 / Â§6.5 | Business continuity (ISO 22301), ISMS/PDCA (ISO 27001), incident response, DoS resilience, security-by-design | Deployment |

**EN 18246 â€” Data authentication, reliability and integrity**

| Clause | Requirement | Status |
|---|---|---|
| Â§4.3 / Â§5.1 | Public read without authentication and without collecting PII | Implemented |
| Â§4.7 | Create/modify on DPP logged non-repudiably, tamper-proof, integrity preserved over time | Implemented |
| Â§4.5, Annex A | ESDC issuance and verification (W3C Verifiable Credential, `application/vc+jwt`) | Implemented |
| Â§4.7, Annex A.3 | Authenticity/integrity verifiable independently, free of charge, without contacting the issuer | Implemented |
...
```
**Note: you must create a user account with set privileges to access the database, and set `Ssl Mode=Require` for any non-loopback server â€” most managed PostgreSQL offerings (including Azure Database for PostgreSQL) reject unencrypted connections outright.**
...
* `Dpp__Esdc__EconomicOperatorId`: The economic operator this server is authorised to sign ESDCs for. **Required in production** &mdash; a server resolving a stable signing key refuses to start without it. Issuance is refused for any DPP naming a different operator, and the server's own signing key is bound to this identifier so it cannot vouch for credentials claiming another operator. See [ESDC issuer binding](#esdc-issuer-binding).
...
* `Dpp__Audit__AllowUnkeyedCheckpointMigration`: Set to `true` only while enabling an audit key over an existing unkeyed log, to let the first keyed append adopt the unkeyed checkpoint. Leave unset otherwise &mdash; while it is on, a downgraded checkpoint is indistinguishable from a legitimately un-migrated one.
...
## Security â€“ STRIDE Threat Analysis (UA-CloudLibrary server)
...
|-------------|---------------|------------------|---------------|
| Brute Force (1M keys) | 16 minutes | 1.7 days | **~99% slower** âœ… |
| DOS (1000 req/sec) | Server overload | Max ~6-7 req/sec | **~99.3% reduction** âœ… |
| Timing Analysis | Exploitable | Fixed timing | **Mitigated** âœ… |

...
> **An unavailable policy source is not an absent policy.** The mapping travels inside the DPP's stored values blob, so "the storage was unreachable" and "this DPP controls nothing" used to arrive at the filter identically, as a null blob &mdash; and only the second means the data is public. Because these reads are anonymous, treating an outage as the latter would publish every controlled element of a protected passport for as long as the database was degraded. `DbFileStorage.DownloadFileAsync` therefore propagates storage faults instead of flattening them to `null`, and `GetControlledElementsAsync` reports them as an unreadable (deny-all) mapping. The same conflation is why exporting or saving a nodeset is refused when its stored values cannot be loaded: writing the browse output alone would silently drop the `controlledElements` map, and re-uploading that file would publish everything it protected.

> **Role filtering descends every element container.** `DataElement` children live under `DataElementCollection.Elements` *or* `MultiValuedDataElement.Value`, and path resolution has always traversed both. The role filter must therefore traverse both as well &mdash; a controlled element nested inside a multi-valued element is addressable through the API, so filtering that skipped it would return the value and sign it into the ESDC. Both now derive their notion of containment from a single helper (`DPPService.ChildElementsOf`), so a future container type cannot be honoured by one and missed by the other.
...
> **Audit reads are logged before the response is returned.** On read paths the ESDC is built first, then the audit entry is appended, and only then does the action return. Building the ESDC first costs nothing in disclosure terms &mdash; it is an in-memory object, and no response bytes are written until the action returns &mdash; while making sure a signing fault surfaces before the log has recorded `Success` for a read that then failed. A failed audit append still turns the whole read into a `503` without handing over the data.

> **A failed audit append is reported differently depending on whether anything was changed.** The two cases are identical to the audit log and opposite for the client:
>
> * **Nothing was applied** (a read, or a mutation whose pre-write `Attempted` entry could not be appended): `503`. Retrying is the correct recovery.

...
> **An unkeyed checkpoint is refused once keyed auditing is enabled.** The checkpoint lives in the database it protects, so "no key was used here" is itself an attacker-controllable claim: clearing its key id and MAC would otherwise skip MAC verification entirely and re-open truncation to a valid prefix even with a key configured. Verification therefore accepts an unkeyed checkpoint only while *every* entry it covers is also unkeyed &mdash; the genuine pre-key state, which the next audited operation re-keys. An unkeyed checkpoint alongside keyed entries is reported as a downgrade attempt, not as a legacy state. The key id is also covered by the checkpoint MAC, so a checkpoint cannot be repointed at a different key while keeping its MAC intact.
...
> **Leaving this unset keeps the previous unbound behaviour**, which is a real weakness rather than a neutral default: any hosted DPP can be signed as whatever operator its content happens to name, and the server's own unbound anchor will then verify it. It is left opt-in only so that upgrading does not break deployments that are already issuing credentials; configure it as part of the upgrade. Peer operators' keys should likewise use the issuer-bound `Dpp__Esdc__TrustedIssuers__*` form rather than the legacy flat `Dpp__Esdc__TrustedPublicKeysPem__*` list, in which any key can vouch for any issuer.
...
* `Dpp__Esdc__EconomicOperatorId`: The economic operator this server is authorised to sign ESDCs for. **Set this in production.** When set, issuance is refused for any DPP naming a different operator, and the server's own signing key is bound to this identifier so it cannot vouch for credentials claiming another operator. When unset the previous unbound behaviour is kept for compatibility, which means a hosted DPP can be signed as whatever operator its content names &mdash; see [ESDC issuer binding](#esdc-issuer-binding).
